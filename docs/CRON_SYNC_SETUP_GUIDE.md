...
メンバーが削除された後に残ってしまう Responses シートの未所属データを、深夜など負荷の低い時間帯にまとめて削除するトリガーを追加します。

### 実装済みの関数

- `scheduledCleanupDetachedResponses()`
  - **用途**: メンバーシートと突き合わせ、`Members` に存在しない `userKey` のレスポンスを一括削除
  - **動作**: シートを読み込み → 未所属レスポンスのみ除外 → シートを書き戻し（ヘッダー装飾も復元）
...
2. **トリガーを追加** をクリック
3. 以下のように設定:
   - **実行する関数を選択**: `scheduledCleanupDetachedResponses`
   - **実行するデプロイを選択**: `Head`
   - **イベントのソースを選択**: `時間主導型`
...
- `adminCleanupDetachedResponses(userKey?, adminToken?)`
