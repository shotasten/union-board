...
#### 3. `scheduledFullSync()` - 1時間ごとの全体同期（フォールバック）
- **用途**: カレンダー更新トリガーが動作しない場合のフォールバック
- **同期方向**:
  - ✅ カレンダー → アプリ（新規イベント検知、イベント情報の変更反映）
  - ✅ アプリ → カレンダー（出欠状況の反映）
- **API呼び出し**: 35-50回/実行（表示期間内の全イベント）
- **注意**: カレンダー更新トリガーが正しく動作している場合は不要

#### 4. `syncResponsesDiffToCalendar()`
- **用途**: 差分検知と双方向同期処理の実行
...
#### トリガー3（オプション）: 1時間ごとの全体同期（フォールバック）

カレンダー更新トリガーが正しく動作しない場合のフォールバックとして設定できます。
...
> **注意**: カレンダー更新トリガーが正しく動作している場合、このトリガーは不要です

#### トリガー4: 5分ごとの高頻度同期（オプション、土日12:40-13:20用）
...
### 旧構成（15分ごと + 1時間ごとcron）との比較

| 構成 | 通常運用時 | 活発な使用時 |
|------|-----------|-------------|
| **推奨構成**（カレンダー更新トリガー） | 500-1500回/日 | 1900-2800回/日 |
| 旧構成（1時間ごとcron） | 1000-2200回/日 | 2400-3360回/日 |
| **削減率** | **約30-50%削減** | **約15-20%削減** |
...
### オプション設定
- [ ] `scheduledFullSync` のトリガーを設定（1時間ごと、フォールバック用）
...
4. **フォールバック**:
   - カレンダー更新トリガーが正しく動作しない場合は、1時間ごとのcron（`scheduledFullSync`）を設定
