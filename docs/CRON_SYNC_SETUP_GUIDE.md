...
メンバーが削除された後に残ってしまう Responses シートの孤児データを、深夜など負荷の低い時間帯にまとめて削除するトリガーを追加します。

### 実装済みの関数

- `scheduledCleanupOrphanResponses()`
  - **用途**: メンバーシートと突き合わせ、`Members` に存在しない `userKey` のレスポンスを一括削除
  - **動作**: シートを読み込み → 孤児レスポンスのみ除外 → シートを書き戻し（ヘッダー装飾も復元）
...
2. **トリガーを追加** をクリック
3. 以下のように設定:
   - **実行する関数を選択**: `scheduledCleanupOrphanResponses`
   - **実行するデプロイを選択**: `Head`
   - **イベントのソースを選択**: `時間主導型`
...
- `adminCleanupOrphanResponses(userKey?, adminToken?)`
