...
**機能概要**:
- ユーザーが希望すれば、楽団カレンダーを自分のGoogleカレンダーに追加できる
- 以降、イベントが自動的に自分のカレンダーに同期される

**フロー**:
1. ユーザーが設定画面で「カレンダーに追加」ボタンをクリック
2. 確認ダイアログ表示（メールアドレス共有の同意）
3. Calendar ACL APIで参照権限（reader）を付与
4. ユーザーのGmailに共有通知が届く
5. 自動的にGoogleカレンダーに「吹奏楽団 イベントカレンダー」が追加
6. 以降、イベントの作成・更新・削除が自動同期

**動作**:
```typescript
// セッションからメールアドレスを取得（一時使用のみ）
...
**プライバシー保護**:
- メールアドレスはSpreadsheetに保存しない
- 共有時のみセッション情報から取得
- UserPreferencesにはuserHashと共有フラグのみ保存
...
**方法1: アプリから解除**
- 設定画面で「共有を解除」ボタンをクリック
- ACLから削除
- UserPreferencesを更新

**方法2: Googleカレンダーから解除**
- Googleカレンダーで「吹奏楽団 イベントカレンダー」のチェックを外す
- または設定から削除
...
// UserPreferences更新
saveUserPreference(userHash, 'calendarShared', false);
```

#### 4.3 匿名モードでの代替機能

**問題**: 匿名モードではメールアドレスがないため、カレンダー共有不可

**代替機能**: iCalファイルダウンロード

**フロー**:
1. イベントカードの「カレンダーに追加」ボタンをクリック
2. .icsファイルがダウンロード
3. ユーザーがファイルを開く
4. Googleカレンダーに手動で追加
...
function listCalendarShares(): Array<SharedUser> {
  ...
  return acl.items
    ...
    .map(item => ({
      email: item.scope.value, // 管理者のみ表示
      userHash: findUserHashByEmail(item.scope.value),
      userName: getUserName(userHash),
      sharedAt: findSharedDate(userHash),
      status: 'active'
    }));
}

// 共有解除（管理者操作）
function adminRevokeCalendarShare(userHash: string) {
  const email = getEmailFromAcl(userHash);
  const rule = findAclRule(email);
...
3. **Sessions** - セッション管理
4. **Config** - 設定情報
5. **UserPreferences** - ユーザー設定
6. **AuditLog** - 監査ログ（オプション）
...
| CALENDAR_ID | `xxx@group.calendar.google.com` | 連携カレンダーID（専用カレンダー） |
| CALENDAR_SHARING_ENABLED | `true` / `false` | カレンダー共有機能ON/OFF |
...
### 5. UserPreferences シート

| カラム | 型 | 説明 | 例 |
|--------|----|----|-----|
| userHash | string | ユーザーハッシュ | `hash-a7f3k9...` |
| calendarShared | boolean | カレンダー共有状態 | `true` / `false` |
| sharedAt | ISO 8601 | 共有開始日時 | `2025-11-06T10:00:00+09:00` |
| updatedAt | ISO 8601 | 更新日時 | `2025-11-06T15:30:00+09:00` |

**インデックス**: userHash（ユニーク）

**用途**:
- カレンダー共有の有効/無効を記録
- メールアドレスは保存しない（プライバシー保護）

### 6. AuditLog シート（オプション）
...
**作成されるカレンダーの特徴**:
- ID形式: `xxx@group.calendar.google.com`
- 管理者アカウントから閲覧・編集可能
- 個人カレンダー（primary）とは別扱い
- 共有ユーザーからは参照のみ（reader権限）
...
|-----------|------|---------|
| 1.0 | 2025-11-06 | 初版作成 |
| 1.1 | 2025-11-06 | 個人カレンダー共有機能追加、専用カレンダー作成仕様追加 |
