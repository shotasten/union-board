...
> **重要**: カレンダーIDで直接追加するには、「一般公開して誰でも利用できるようにする」のチェックを**入れる必要があります**。チェックを外すと、カレンダーIDを知っていても「アクセスする権限がありません」というエラーになります。

1. **☑️ 「一般公開して誰でも利用できるようにする」にチェックを入れる**
   - これにより、カレンダーIDを使った追加が可能になります
   - プルダウンメニューが選択可能になります

2. **権限レベルを設定**:
   - プルダウンで **「予定の表示（すべての予定の詳細）」** を選択
   - これにより、イベント名、場所、説明が表示されます
   - ⚠️ 注意: この設定では、カレンダーIDを知っている人は誰でも追加できますが、編集はできません（閲覧のみ）
...
**注意点**:
- ⚠️ 一般公開のため、カレンダーIDを知っている人は誰でも追加できます
- ⚠️ 検索エンジンで見つかる可能性があります（ただし、カレンダーIDを知らない限り追加は困難）
- ✅ 閲覧のみ可能で、編集はできません
...
> **補足**: 
> - リンクで使われているID（Base64エンコードされた形式）は、このカレンダーIDをエンコードしたものです
> - アプリはConfigシートからカレンダーIDを取得して使用します
> - URLエンコード（`@` → `%40`）も自動的に行われます
...
**推奨**: カレンダーID方式（オプションA）
- 「一般公開」のチェックを入れる（必須）
- 権限レベルを「予定の表示（すべての予定の詳細）」に設定
- カレンダーIDを確認（アプリで使用）
- アプリの「カレンダーに追加」ボタンでカレンダーIDを使って追加

> **補足**: 
> - 「共有可能なリンクを取得」ボタンは使わなくてOKです。カレンダーIDがあれば追加できます。
> - 「一般公開」のチェックを外すと、カレンダーIDで追加できません（「アクセスする権限がありません」エラー）。
...
**カレンダーID方式を使用する場合、「一般公開して誰でも利用できるようにする」のチェックを入れる必要があります。**

- チェックを外す = カレンダーIDで追加できない（「アクセスする権限がありません」エラー）
- チェックを入れる = カレンダーIDで追加可能
  - カレンダーIDを知っている人は誰でも追加できます
  - 検索エンジンで見つかる可能性があります
  - ただし、閲覧のみ可能で編集はできません
