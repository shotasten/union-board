...
**機能**:
- タイトル・日時・場所が一致する重複イベントを自動検出
- 最新の更新日時を持つイベントを残し、他を削除
- 過去90日〜未来1年の範囲を対象

**使用方法**:
```typescript
// GASエディタで実行
adminRemoveDuplicateCalendarEvents()
```
...
🔍 [重複検出] 冬合宿（わくビレ） (10件)
  ✅ [削除] xxx@google.com
  ...
✅ [重複削除完了] 25件の重複を削除しました
```

**重複防止機能**:
- 同期時にタイトル・日時・場所で既存イベントを検索
- IDで見つからない場合もフォールバック検索
- 見つかったIDをスプレッドシートに自動保存
...
| CALENDAR_SHOW_PART_BREAKDOWN | `true` / `false` | カレンダー説明欄にパート別内訳を表示（デフォルト: `false`） |
...
**基本形式**（`CALENDAR_SHOW_PART_BREAKDOWN = false` の場合）:
```
（ユーザーが入力した説明）
...
> **備考**: アプリUI上のステータス表記は「△ 遅刻/早退」ですが、Googleカレンダーの説明欄では文字数削減のため「△ 遅早」と表示します。

**パート別内訳を含む形式**（`CALENDAR_SHOW_PART_BREAKDOWN = true` の場合）:
```
（ユーザーが入力した説明）
...
【パート別内訳】
○ (出席) の内訳
Fl (5人): 田中、佐藤、山田、鈴木、高橋
Ob (3人): 中村、小林、加藤
Cl (4人): 伊藤、吉田、渡辺、中島
Sax (6人): 松本、井上、木村、林、斎藤、清水
Hr (2人): 山口、森
Tp (4人): 池田、橋本、石井、前田
Tb (2人): 小川、藤田
Bass (2人): 岡田、長谷川
...
**パート別内訳の設定方法**:
- Configシートの `CALENDAR_SHOW_PART_BREAKDOWN` を `true` に設定すると有効化
- デフォルトは `false`（基本形式のみ表示）
- 設定変更後、次回のカレンダー同期時に反映される
- パート別内訳を追加すると説明欄が長くなる（約1,500-2,500文字）ため、可読性を考慮して設定すること
...
重大エラー（認証切れ、カレンダーID無効など）:
- 全体同期を即座に中断
- 管理者に通知（ログに記録）
- カレンダーIDエラーの場合: ConfigシートのCALENDAR_IDを確認するよう指示

カレンダーID関連エラー:
- CALENDAR_IDが未設定: エラーで終了（自動作成しない）
- CALENDAR_IDが無効: エラーで終了（自動作成しない）
- 対処法: setupBandCalendar()を実行 or 正しいIDを設定
```
