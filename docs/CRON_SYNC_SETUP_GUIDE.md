...
- **用途**: 15分ごとまたは5分ごとのトリガーで実行可能な統一関数
- **重複防止**: 10分以内の再実行を自動的にスキップ
- **動作**: 複数のトリガーから呼ばれても、重複実行を自動的に防止

#### 2. `syncResponsesDiffToCalendar()`
- **用途**: 差分検知と同期処理の実行
...
同じ関数 `scheduledSyncResponsesToCalendar` に対して、複数のトリガーを設定します。
...
#### トリガー2: 5分ごとの高頻度同期（土日12:40-13:20用）

1. GAS エディタで **トリガー（時計アイコン）** をクリック
2. **トリガーを追加** をクリック
3. 以下のように設定:
   - **実行する関数を選択**: `scheduledSyncResponsesToCalendar`（同じ関数）
   - **実行するデプロイを選択**: `Head`
   - **イベントのソースを選択**: `時間主導型`
   - **時間ベースのトリガーのタイプを選択**: `分ベースのタイマー`
   - **時間の間隔を選択（分）**: `5分ごと`
...
### 重複防止の仕組み
- 同じ関数 `scheduledSyncResponsesToCalendar` が複数のトリガーから呼ばれても、前回同期から10分以内の場合は自動的にスキップされます
