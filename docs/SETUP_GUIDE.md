...
- ConfigシートのCALENDAR_IDには、作成されたカレンダーのIDが自動的に保存されます
- **このIDを変更・削除しないでください**
- もし誤って変更した場合：
  1. Googleカレンダーで該当カレンダーの設定を開く
  2. 「カレンダーの統合」セクションでカレンダーIDをコピー
  3. ConfigシートのCALENDAR_IDに貼り付ける
- **無効なIDを設定すると、同期がエラーで停止します**（自動的に新しいカレンダーは作成されません）
