...
**エラーメッセージ**: `ConfigシートにCALENDAR_IDが設定されていません` または `カレンダーの取得に失敗しました`

**原因**: ConfigシートのCALENDAR_IDが未設定、または無効なIDが設定されている

**対処法**:
1. **Googleカレンダーで正しいカレンダーIDを確認**
   - Googleカレンダーを開く
...
2. **ConfigシートのCALENDAR_IDに貼り付け**
   ...
   - CALENDAR_IDの行のB列（Value列）に貼り付け

3. **新しいカレンダーを作成する場合**
   - GASエディタで `setupBandCalendar` 関数を実行
   - 自動的にCALENDAR_IDが設定されます

⚠️ **注意**: カレンダーIDが無効な場合、同期は実行されません。エラーログを確認して正しいIDを設定してください。

### 同じイベントが複数作成される（重複イベント）

**症状**: カレンダーに同じイベントが何個も表示される

**原因**: 
- 過去のバージョンで自動同期時に重複チェックが不十分だった
- ConfigシートのCALENDAR_IDを編集中に同期が実行された

**対処法**:

1. **重複イベントを自動削除する（推奨）**
   - GASエディタを開く
   - 関数選択で `adminRemoveDuplicateCalendarEvents` を選択
   - 「実行」ボタンをクリック
   - ログで削除された件数を確認
   - 例: `✅ [重複削除完了] 25件の重複を削除しました`

2. **手動削除**
   - Googleカレンダーで重複イベントを1つずつ削除
   - 1件だけ残して他を削除

**重複防止**:
- ✅ 最新版では重複チェック機能が実装済み
- ✅ ConfigシートのCALENDAR_IDは正しいIDを設定してください
- ✅ 無効なIDを設定すると、エラーで同期が停止します（自動作成されません）
